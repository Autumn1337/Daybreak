...
from .scrapers.wallstreetcn import WallStreetCNScraper
...
class DaybreakOrchestrator:
    ...
    async def run(self, force_hours: int = None) -> None:
        ...
        """
        self.console.print("[bold cyan]🌅 Daybreak - Starting aggregation...[/bold cyan]\n")
        ...
        try:
            ...
            for lang in self.config.ai.languages:
                ...
                try:
                    docs_path = self.storage.save_docs_post(today, summary, language=lang)
                    self.console.print(f"📄 Copied {lang.upper()} summary to GitHub Pages: {docs_path}\n")
            ...
            if self.config.feishu and self.config.feishu.enabled:
                ...
                if feishu_ok:
                    self.console.print("   ✅ Feishu notification sent\n")
                else:
                    self.console.print("[yellow]   ⚠️ Feishu notification failed[/yellow]\n")

            self.console.print("[bold green]✅ Daybreak completed successfully![/bold green]")
    ...
    async def fetch_all_sources(self, since: datetime) -> List[ContentItem]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if self.config.sources.producthunt.enabled:
                ph_scraper = ProductHuntScraper(self.config.sources.producthunt, client)
                tasks.append(self._fetch_with_progress("Product Hunt", ph_scraper, since))

            # 36Kr
            if self.config.sources.kr36.enabled:
                kr36_scraper = Kr36Scraper(self.config.sources.kr36, client)
                tasks.append(self._fetch_with_progress("36Kr", kr36_scraper, since))

            # WallStreetCN
            if self.config.sources.wallstreetcn.enabled:
                wscn_scraper = WallStreetCNScraper(self.config.sources.wallstreetcn, client)
                tasks.append(self._fetch_with_progress("WallStreetCN", wscn_scraper, since))
    ...
    async def _analyze_content(self, items: List[ContentItem]) -> List[ContentItem]:
        ...
        ai_client = create_ai_client(self.config.ai)
        # Use first configured language for summary generation
        lang = self.config.ai.languages[0] if self.config.ai.languages else "en"
        analyzer = ContentAnalyzer(ai_client, language=lang)
