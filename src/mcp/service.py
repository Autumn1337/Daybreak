"""Application service for staged Daybreak pipeline execution."""
...
class DaybreakPipelineService:
    ...
    def get_run_stage(
        ...
        if max_items <= 0:
            raise DaybreakMcpError(code="HZ_INVALID_INPUT", message="max_items must be greater than 0.")
        try:
    ...
    def get_effective_config(
        ...
        return {
            "daybreak_path": str(ctx.daybreak_path),
    ...
    async def fetch_items(
        ...
        if hours <= 0:
            raise DaybreakMcpError(code="HZ_INVALID_INPUT", message="hours must be greater than 0.")
    ...
    async def score_items(
        ...
        if not items:
            raise DaybreakMcpError(code="HZ_EMPTY_INPUT", message="No items available for scoring.")
    ...
    async def filter_items(
        ...
        if topic_dedup and important_items:
            storage = make_storage(ctx.runtime, ctx.config_path)
            orchestrator = make_orchestrator(ctx.runtime, ctx.config, storage)
            result = orchestrator.merge_topic_duplicates(important_items)
            import asyncio
            if asyncio.iscoroutine(result):
                important_items = await result
            else:
                important_items = result
    ...
    async def enrich_items(
        ...
        if not items:
            raise DaybreakMcpError(code="HZ_EMPTY_INPUT", message="No items available for enrichment.")
    ...
    async def run_pipeline(
        ...
        for lang in final_languages:
            summary_result = await self.generate_summary(
                ...
                daybreak_path=daybreak_path,
                config_path=config_path,
                save_to_daybreak_data=save_to_daybreak_data,
            )
    ...
    def _build_context(
        self,
        daybreak_path: str | None,
        config_path: str | None,
        sources: list[str] | None,
    ) -> tuple[PipelineContext, list[str], list[str]]:
        resolved_daybreak = resolve_daybreak_path(daybreak_path)
        runtime = load_runtime(resolved_daybreak)
        resolved_config = resolve_config_path(resolved_daybreak, config_path)
        ...
        return (
            PipelineContext(
                daybreak_path=resolved_daybreak,
    ...
    def _load_stage_items(
        ...
        ctx, _, _ = self._build_context(daybreak_path=daybreak_path, config_path=config_path, sources=None)
        try:
