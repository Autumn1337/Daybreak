# Daybreak MCP Integration

## Recommended Command

Start the built-in MCP server from the Daybreak repository root:
...
Some MCP clients need a fixed working directory in their config. In that case, the absolute path is only used in the client-side `cwd` field, not in Daybreak's code.
...
```json
{
  "mcpServers": {
    "daybreak": {
      "command": "uv",
      "args": ["run", "daybreak-mcp"],
      "cwd": "/absolute/path/to/Daybreak"
    }
  }
}
```
...
```json
{
  "DAYBREAK_MCP_SECRETS_PATH": "/absolute/path/to/mcp.secrets.json"
}
```
