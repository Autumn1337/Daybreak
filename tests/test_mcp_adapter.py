...
from src.mcp.daybreak_adapter import _load_mcp_secrets, resolve_config_path, resolve_daybreak_path


def test_resolve_daybreak_path_accepts_explicit_repo() -> None:
    repo_root = Path(__file__).resolve().parents[1]

    assert resolve_daybreak_path(str(repo_root)) == repo_root.resolve()
...
def test_load_mcp_secrets_loads_generic_env_keys(tmp_path: Path, monkeypatch) -> None:
    ...
    repo_root = Path(__file__).resolve().parents[1]
    monkeypatch.setenv("DAYBREAK_MCP_SECRETS_PATH", str(secrets_path))
