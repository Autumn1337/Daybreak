...
- `data/config.example.json` — 配置模板（git 跟踪，占位符值）
- `data/config.json` — 真实运行时配置（gitignored，含代理地址和 webhook）
...
AI 评分默认并发度为 5（`analyzer.analyze_batch(concurrency=5)`），代理 rate limit 甜点。
...
4. 在 `mcp/daybreak_adapter.py` 的 `VALID_SOURCES` 和 `apply_source_filter` 中注册
...
基于 [Thysrael/Horizon](https://github.com/Thysrael/Horizon) 二次开发（保留出处），已独立为 [Autumn1337/Daybreak](https://github.com/Autumn1337/Daybreak)。
