#!/usr/bin/env bash
# Daybreak daily run + deploy to GitHub Pages
# Usage: ./scripts/daily-run.sh
# Cron:  0 8 * * * /path/to/daybreak/scripts/daily-run.sh >> /path/to/daybreak/logs/cron.log 2>&1
...
echo "$LOG_PREFIX Starting Daybreak daily run..."
