...
[![Daily Summary](https://github.com/Autumn1337/Daybreak/actions/workflows/deploy-docs.yml/badge.svg?style=flat-square)](https://autumn1337.github.io/Daybreak/)
[![GitHub commit activity](https://img.shields.io/github/commit-activity/m/Autumn1337/Daybreak?style=flat-square)](https://github.com/Autumn1337/Daybreak/commits/main)
...
Daybreak 从多个可自定义的信息源中收集新闻，利用 AI 对新闻进行打分与过滤，最终生成一份包含摘要、社区讨论和背景知识的中英双语日报。

[📖 在线演示](https://autumn1337.github.io/Daybreak/) · [📋 配置指南](https://autumn1337.github.io/Daybreak/configuration) · [English](README.md)
...
Daybreak 通过 **AI 自动打分过滤** + **背景知识补充** + **社区评论汇总** + **中英双语输出**，将每日信息获取从"大海捞针"变为"开箱即读"。
...
```bash
git clone https://github.com/Autumn1337/Daybreak.git
cd Daybreak
...
docker-compose run --rm daybreak

# 或自定义时间窗口
docker-compose run --rm daybreak --hours 48
```
...
```bash
uv run daybreak-wizard
```

向导会询问你的兴趣（如"LLM 推理"、"嵌入式"、"web 安全"），自动从[预设源库](https://autumn1337.github.io/Daybreak/presets)匹配并生成 `data/config.json`，还可选让 AI 补充推荐小众源。
...
```bash
uv run daybreak              # 使用默认 24 小时窗口
uv run daybreak --hours 48   # 抓取最近 48 小时的内容
```

#### 使用 Docker

```bash
docker-compose run --rm daybreak              # 使用默认 24 小时窗口
docker-compose run --rm daybreak --hours 48   # 抓取最近 48 小时的内容
```
...
Daybreak 非常适合作为 **GitHub Actions** 定时任务运行。查看 [`.github/workflows/daily-summary.yml`](.github/workflows/daily-summary.yml) 获取现成的工作流配置，可自动生成日报并部署到 GitHub Pages。
...
Daybreak 内置了 [MCP](https://modelcontextprotocol.io/) Server，AI 助手可以直接通过它驱动抓取、打分、过滤、富化和摘要流程。
...
Daybreak 的配置向导使用社区维护的[预设源库](https://autumn1337.github.io/Daybreak/presets)来推荐信息源。**我们非常欢迎你来扩充它！**
